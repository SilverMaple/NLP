...
base_dir = 'nlp-data'
train_set_file = os.path.join(base_dir, 'small_train_set.csv')
...
def evaluate(sess, x_, y_, wv_model=None):
    '''评估数据的准确率和损失'''
    data_len = len(x_)
    batch_eval = batch_iter_jit(x_, y_, config.batch_size)
    total_loss = 0.0
    total_acc = 0.0
...
def train2(restore=False):
    ...
    for epoch in range(1, config.num_epochs+1):
        ...
        batch_train = batch_iter_jit(x_train, y_train, config.batch_size)
        # batch_train = batch_iter(x_train, y_train, config.batch_size)
        # logging.info('batch data fetch success.')
        # logging.info(type(batch_train))
        total_batch = 0
