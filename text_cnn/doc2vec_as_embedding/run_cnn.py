...
from utils.doc2vec import Manager
...
def evaluate(sess, x_, y_, dv_model=None):
    '''评估数据的准确率和损失'''
    data_len = len(x_)
    batch_eval = batch_iter_jit(x_, y_, dv_model, config.batch_size)
    total_loss = 0.0
    total_acc = 0.0
...
def get_docvec_model():
    ...
    manager = Manager(retrain=True, min_count=min_count, size=size, window=window, epoch=epoch,
                      learning_rate=learning_rate)
    return manager.load_model("cache/train_doc2vec.model")
...
def train():
    ...
    flag = False
    dv_model = get_docvec_model()
    # wv_model = get_wordvec_model()
    for epoch in range(config.num_epochs):
        logging.info('Epoch: {}'.format(epoch + 1,))
        batch_train = batch_iter_jit(x_train, y_train, dv_model, config.batch_size)
        ...
        for x_batch, y_batch in batch_train:
            ...
            if total_batch % config.print_per_batch == 0:
                ...
                loss_val, acc_val = evaluate(session, x_val, y_val, dv_model=dv_model)
